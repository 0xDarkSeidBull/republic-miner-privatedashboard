<script>
  ...
  async function loadGeo() {
    ...
    geoError = '';
    try {
      const r = await fetch(`${API}/api/weekly/geo/fast`, { signal: AbortSignal.timeout(15000) });
      if (!r.ok) throw new Error('Failed');
      const d = await r.json();
      geoData = d.data || [];
      geoMeta = { week: d.week, total_jobs: d.total_jobs, miners: d.miners, pool: d.pool, updated_at: d.updated_at };
  ...
  async function triggerScan() {
    ...
    geoError = '';
    try {
      await fetch(`${API}/api/weekly/geo/refresh`, { method: 'POST' }).catch(() => {});
      await new Promise(r => setTimeout(r, 95000));
      await loadGeo();
    } catch(e) {
      geoError = 'Scan failed.';
      geoLoading = false;
    }
  }
  ...
  onMount(() => { loadGeo(); interval = setInterval(loadGeo, 10 * 60 * 1000); });
...
<div style="max-width:1224px;margin:0 auto;padding:0 28px 16px">
  <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:16px 24px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
    <span style="font-size:13px;color:var(--muted)">⚡ Fast fetch — scans all validators in ~90 sec instead of waiting for full job pagination</span>
    <span style="font-size:12px;color:var(--accent3);margin-left:auto">Last scan: {geoMeta.updated_at ? new Date(geoMeta.updated_at).toLocaleString() : '—'}</span>
  </div>
</div>
...
<div style="max-width:1224px;margin:0 auto;padding:0 28px 24px">
  <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:20px 24px">
    ...
    <div style="display:flex;justify-content:space-between;margin-top:6px;font-size:11px;color:var(--muted)">
      <span>Sun 00:00 UTC</span><span>Sat 23:59 UTC</span>
    </div>
  </div>
</div>
...
<div class="stats-grid" style="margin-top:0">
  ...
  <div class="stat-card">
    ...
    <div class="stat-sub">Auto-refresh 10 min</div>
  </div>
  <div class="stat-card">
...
<div class="section">
  <div class="section-header">
    ...
    <button class="refresh-btn" on:click={triggerScan} disabled={geoLoading}>
      {geoLoading ? '⏳ Scanning... (~90s)' : '⚡ Fresh Scan'}
    </button>
  </div>

  {#if geoLoading && geoData.length === 0}
    <div class="loading" style="padding:60px;text-align:center">Loading scores...</div>
  {:else if geoError}
  ...
  {:else}
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          {#if geoData.length === 0}
            <tr><td colspan="11" class="loading" style="padding:40px;text-align:center">No data yet!</td></tr>
          {:else}
            {#each geoData as m}
