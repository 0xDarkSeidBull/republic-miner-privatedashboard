<script>
  ...
  const REPUBLIC_CHAIN = {
    ...
    currencies: [{ coinDenom: 'RAI', coinMinimalDenom: 'arai', coinDecimals: 18, coinGeckoId: '' }],
    feeCurrencies: [{ coinDenom: 'RAI', coinMinimalDenom: 'arai', coinDecimals: 18, coinGeckoId: '', gasPriceStep: { low: 10000000000, average: 25000000000, high: 40000000000 } }],
  ...
  async function submitJob() {
    ...
    try {
      const r = await fetch(`${API}/api/hyperscale/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: prompt.trim(), miner_address: selectedMiner?.address || '', model: selectedModel }),
      });
      const d = await r.json();
      if (!d.success) throw new Error(d.error || 'Failed');
      trackingId = d.tracking_id;
      pollInterval = setInterval(pollStatus, 3000);
    } catch(e) { error = e.message; loading = false; paymentStep = 'idle'; }
  }
  ...
  async function pollStatus() {
    ...
    try {
      ...
      if (d.status === 'completed' || d.status === 'inferred_only' || d.status === 'failed') {
        ...
        result = d; loading = false; paymentStep = 'idle';
      }
    } catch(e) {}
  }

  function reset() {
    result = null; error = ''; trackingId = null; prompt = ''; loading = false;
    selectedMiner = null; selectedModel = 'nex-agi/deepseek-v3.1-nex-n1';
  ...
  <div style="background:var(--bg2);border:1px solid {keplrConnected ? 'rgba(74,222,128,.3)' : 'var(--border)'};border-radius:12px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
    <div>
      ...
      {#if keplrConnected}
        ...
        <div style="font-family:var(--font-mono);font-size:11px;color:var(--muted);margin-top:2px">Balance: <span style="color:#4ADE80;font-weight:700">{userBalance} RAI</span></div>
      {:else}
    ...
    {:else}
      <div style="font-family:var(--font-mono);font-size:11px;color:var(--muted)">Fee: <span style="color:var(--accent);font-weight:700">{RAI_FEE} RAI</span> per inference</div>
    {/if}
  </div>
  ...
  {#if !result}
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:24px">
      ...
      <div style="margin-bottom:20px">
        <div style="font-family:var(--font-mono);font-size:11px;color:var(--muted);margin-bottom:8px;letter-spacing:1px">SELECT MODEL <span style="color:var(--accent)">({models.length} available)</span></div>
        {#if modelsLoading}
          <div style="font-family:var(--font-mono);font-size:11px;color:var(--muted)">Loading models...</div>
        {:else}
          <input bind:value={modelSearch} placeholder="Search models... e.g. gpt, llama, deepseek"
          ...
          <select bind:value={selectedModel} size="5"
            ...
            {#each filteredModels as model}
              <option value={model.id} style="background:#0D0D1A;color:#E8E8F0;padding:6px">{model.name} — {model.id}</option>
            {/each}
          </select>
