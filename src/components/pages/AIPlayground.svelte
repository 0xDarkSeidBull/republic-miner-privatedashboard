<script>
  ...
  // Convert RAI to arai (smallest unit)
  function raiToArai(amountInRAI) {
    // Use string manipulation to avoid BigInt precision issues
    const parts = amountInRAI.toString().split('.');
    let whole = parts[0] || '0';
    let decimal = parts[1] || '';
    decimal = decimal.padEnd(18, '0').slice(0, 18);
    return BigInt(whole) * BigInt(10n ** 18n) + BigInt(decimal);
  }
  ...
  async function connectKeplr() {
    ...
    try {
      if (!window.keplr) {
        throw new Error('Keplr wallet not found! Please install Keplr extension from keplr.app');
      }
      ...
      try {
        ...
        const balanceResponse = await client.getBalance(userAddress, 'arai');
        const balBigInt = BigInt(balanceResponse.amount);
        userBalance = (Number(balBigInt / BigInt(10n ** 14n)) / 10000).toFixed(4);
      } catch (balErr) {
  ...
  async function payAndInfer() {
    ...
    try {
      ...
      // Prepare amount
      const araiAmount = raiToArai(RAI_FEE).toString();
      const amount = [{ denom: 'arai', amount: araiAmount }];
      const fee = {
        amount: [{ denom: 'arai', amount: '200000000000000' }],
        gas: '200000',
      };

      // Send tokens — this triggers Keplr popup
      ...
      try {
        const queryClient = await SigningStargateClient.connect(RPC_URL);
        const balanceResponse = await queryClient.getBalance(userAddress, 'arai');
        const balBigInt = BigInt(balanceResponse.amount);
        userBalance = (Number(balBigInt / BigInt(10n ** 14n)) / 10000).toFixed(4);
      } catch (e) {
      ...
      await new Promise((r) => setTimeout(r, 5000)); // Wait for tx to be indexed
